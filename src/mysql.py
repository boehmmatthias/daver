...
def get_table_definition(table_name, connection_params, method="describe"):
    """
    ...
    try:
        ...
        print(f"Error getting table definition for {table_name}: {err}")
        return None
...
def get_knowledge_base_schema_for_table(table_name, db_connection_params):
    definition = get_table_definition(table_name, db_connection_params, method="describe")
    
    fields = []
    for row_t in definition.iterrows():
        row = row_t[1]
        fields.append({'name': row['Field'], 'type': row['Type'], 'nullable': row['Null'], 'key': row['Key'], 'default': row['Default'], 'extra': row['Extra'], 'context': ''})
